...
- **VPS / production mode:** system service (`sudo systemctl`) — the package ships only inert reference text under `references/systemd/`, never an auto-activated `.service`; generate/copy your own unit file (see notes below).
...
Recommended: generate the user unit + env file via the built-in helper (resolves the install path and writes the unit for you):
...
- Systemd templates are provided under `references/systemd/` for reference (inert `.txt`); the live unit is the one the installer writes.
...
We intentionally **do not ship** an auto-activated `systemd/system/*` unit (scanner/persistence concerns); the only system-scope artifact is the reference skeleton below.
